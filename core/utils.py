...
def generate_random_gravity_measurement(base_gravity=9.81, noise_level=0.05):
    """
    Generate a random gravity measurement with noise.

    :param base_gravity: The base gravity value (default is Earth's gravity).
    :param noise_level: The level of noise to add to the measurement.
    :return: A simulated gravity measurement.
    """
    noise = random.uniform(-noise_level, noise_level)
    measurement = base_gravity + noise
    logger.info("Generated random gravity measurement: %.4f m/s^2", measurement)
    return measurement
...
def load_json_file(filename):
    ...
    try:
        with open(filename, 'r') as file:
            data = json.load(file)
            logger.info("Data loaded successfully from %s", filename)
            return data
    except json.JSONDecodeError as e:
        logger.error("Error decoding JSON file: %s", e)
        raise
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # Generate a random gravity measurement
    gravity_measurement = generate_random_gravity_measurement()
    print(f"Random gravity measurement: {gravity_measurement:.4f} m/s^2")
